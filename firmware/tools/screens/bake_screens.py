...
from featherframe.render import system, theme, typography  # noqa: E402  (needs sys.path)
...
def band(y0, y1):
    ...
    its rows are native columns, and a window must start on a whole byte."""
    return y0 & ~7, (y1 + 7) & ~7
...
def _draw_error_pill(d, text, kind):
    fnt = sans(PILL_TEXT_SIZE)
    tw = d.textlength(text, font=fnt)
    pillw = int(PILL_PAD + ERR_ICON_SLOT + PILL_GAP + tw + PILL_PAD + 4)
    px = int(W / 2 - pillw / 2)
    cy = PILL_Y + PILL_H / 2
...
def error_assets():
    ...
    band_ = _aligned_region(boot + pills, *band(PILL_Y - 4, PILL_Y + PILL_H + 4))
    err_geo, err_tiles = _region_tiles(band_, pills)
...
TOAST_Y = PILL_Y          # the boot pills' own line
...
TOASTS = [
    ...
    # Not a button toast: painted once when the cell crosses the low-battery
    # threshold, and left on the glass through the hold (W-736).
    ("LOW_BATTERY",    LOW_BATTERY_TEXT, "battery"),
]
...
def _draw_toast(d, text, style):
    ...
    if style == "wifi":
        ...
        # chip, inverted.
        slot = round(PILL_H * 0.58)
        pillw = int(PILL_PAD + slot + PILL_GAP + tw + PILL_PAD + 4)
        px = int(W / 2 - pillw / 2)
        d.rounded_rectangle([px, TOAST_Y, px + pillw, TOAST_Y + PILL_H],
                            radius=PILL_H / 2, fill=0)
        wifi_glyph(d, px + PILL_PAD + slot / 2, cy + PILL_H * 0.1, PILL_H * 0.23, fill=255)
    ...
    d.rounded_rectangle([px, TOAST_Y, px + pillw, TOAST_Y + PILL_H],
                        radius=PILL_H / 2, fill=0)
    if style == "done":
        check(d, px + PILL_PAD + LOADER_SLOT_W / 2, cy, PILL_H * 0.32, width=max(3, round(PILL_H * 0.07)))
    else:
...
def toast_assets():
    ...
    for _, text, style in TOASTS:
        ...
        canvases.append(im)
    geo, tiles = _region_tiles(_aligned_region(canvases, *band(PILL_Y - 4, PILL_Y + PILL_H + 4)),
                               canvases + [Image.new("L", (W, H), 255)])
...
def _mat_macros(up_w, up_h, foot_cy, native):
    ...
    cx, cy = up_w // 2 & ~7, up_h // 2 & ~7
    base, sx, sy = native((cx, cy, 8, 8)), native((cx + 8, cy, 8, 8)), native((cx, cy + 8, 8, 8))
    unit = lambda a, b: (b - a) // 8
    return ["// The footer line toasts sit on, and the mat that moves it (X-FF-Mat).",
            "// A toast is baked where the line lands under FF_REF_INSET; the firmware",
            "// shifts the window by where it lands under the frame's own mat.",
            f"#define FF_REF_INSET      {REF_INSET:.1f}f",
            ...
            f"#define FF_UPY_NX         {unit(base[0], sy[0])}",
            f"#define FF_UPY_NY         {unit(base[1], sy[1])}", ""]
...
# refresh longer than the stage it names, so a healthy boot says "Connecting"
# once and only a failure gets specific (W-817). The firmware shows BOOT_WIFI
...
def write_header_full(t):
    ...
    L += ["// Stamp tiles (PackBits, black/white ink codes, native coords): the firmware",
          ...
          *_mat_macros(t.w, t.h, t.at[1] + system.NOTE_CY * t.scale, t.native_window),
          ...
          f"#define FF_CORNER_X       {cwin[0]}", f"#define FF_CORNER_Y       {cwin[1]}",
          f"#define FF_CORNER_W       {cwin[2]}", f"#define FF_CORNER_H       {cwin[3]}",
          f"#define FF_MAX_TILE_BYTES {max(twin[2] * twin[3], cwin[2] * cwin[3]) // 2}", ""]
    trefs = []
    for (name, *_rest), pb in zip(TOASTS, toasts):
        if pb is None:
            trefs.append(("nullptr", 0)); continue
        arr = f"ff_toast_{name.lower()}"; trefs.append((arr, len(pb)))
        _c_array(L, arr, pb)
    trefs.append(("nullptr", 0))                       # FF_TOAST_BLANK: a clean repaint instead
    crefs = []
    for i, pb in enumerate(corners):
        arr = f"ff_corner_{i}"; crefs.append((arr, len(pb)))
        _c_array(L, arr, pb)
    L += ["static const FfScreenAsset ff_toast_tiles[FF_TOAST_COUNT] = {"]
    L += [f"  {{ {arr}, {ln} }}," for arr, ln in trefs]
    L += ["};", "static const FfScreenAsset ff_corner_tiles[2] = {"]
    L += [f"  {{ {arr}, {ln} }}," for arr, ln in crefs]
    L += ["};", "",
