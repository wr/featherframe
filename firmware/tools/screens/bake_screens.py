...
def screen_boot(text, bird=False):
    ...
    d.rounded_rectangle([px, py, px + pillw, py + pillh], radius=pillh / 2, fill=30)
...
def screen_setup():
    ...
    d.rounded_rectangle([x0, y0, x1, y1], radius=24, fill=30)
    steps = [
...
def write_header():
    L = ["// GENERATED by firmware/tools/screens/bake_screens.py — do not edit by hand.",
         "// Boot + first-time-setup panel screens, baked at 1404x1872, 1-bit.",
         "#pragma once", "#include <stdint.h>", "",
         f"#define FF_SCREEN_W       {W}", f"#define FF_SCREEN_H       {H}",
         f"#define FF_SCREEN_STRIDE  {STRIDE}",
         f"#define FF_SCREEN_BYTES   {NBYTES}   // decoded size, per screen", "",
         "enum FfScreen {"]
    ...
    for arr, ln in refs:
        L.append(f"  {{ {arr}, {ln} }},")
    L += ["};", "",
          "// PackBits decode: expand one screen into a caller-provided buffer of",
          "// FF_SCREEN_BYTES. Returns bytes written (== FF_SCREEN_BYTES on success).",
    ...
    print(f"wrote {OUT_H}: {len(SCREENS)} screens, {total/1024:.0f}K packed")
